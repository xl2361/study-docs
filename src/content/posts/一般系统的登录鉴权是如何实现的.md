...
不同的登录认证方式，获取用户的实现也不同，知道一般思路即可。真实工作中有现成的，也可以让 AI 写。

一般思路如下：

拦截请求 → 解析凭证中的用户信息 → 存入当前线程 → 后续随时调用

**1）、拦截请求**

请求到达后端，会先经过全局拦截器(Filter / Interceptor)。它会检查请求中有没有Token、SessionID等登录凭证。
...
把用户信息存入ThreadLocal (如 Spring Security 的 SecurityContext)。后续在Controller、Service中调用就行：
