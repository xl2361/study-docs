...
let globalIconObserver: MutationObserver | null = null;
...
export function initIconLoader(): void {
	...
	// 监听新添加的图标
	if (window.MutationObserver && !globalIconObserver) {
		globalIconObserver = new MutationObserver((mutations) => {
		...
		globalIconObserver.observe(document.body, {
			childList: true,
			subtree: true,
		});
	}
