...
因为在真实工作中，很多中间件以及我们的应用服务，通常都是部署在 **Linux 服务器** 上，而不是 Windows 系统。 所以，Linux 的常见操作是开发人员必须掌握的基础能力。

另外，Linux 的使用方式和 Windows 也不太一样。 Windows 更多是通过图形化界面进行管理， 而 Linux 在实际工作中，更多是通过**命令行**来完成文件查看、日志排查、服务启动、进程管理等操作。

因此，想要更好地适应真实开发工作， Linux 的基本使用一定要会。
...
| /opt                | 存放额外安装的软件，相当于Windows系统的Program Files目录 |
...
| 模式    | 说明        | 常用操作                                      |
| ----- | --------- | ----------------------------------------- |
| 一般模式  | 打开文件的默认模式 | gg：跳到文件首行G：跳到文件末行yy：复制当前行p：粘贴dd：删除当前行u：撤销 |
...
| find /opt -name "\*.java" | grep user | 查找opt目录下所有含user名称的 .java文件 |
...
| 命令                 | 说明                     |
| ------------------ | ---------------------- |
| gzip file.txt      | 压缩单个文件，生成.gz压缩包，并删除原文件 |
| gunzip file.txt.gz | 解压.gz压缩包，并删除原压缩包       |
...
| yum remove firefox.x86\_64 | 卸载软件         |
| yum update                 | 更新所有软件包      |
...
| 命令                                                              | 说明                                    |
| --------------------------------------------------------------- | ------------------------------------- |
| ping [www.baidu.com](http://www.baidu.com)                      | 测试网络连通性                               |
| telnet ip port                                                  | 测试端口是否连通（如：telnet 192.168.1.100 3306） |
| curl [http://api.example.com](http://api.example.com)           | 测试HTTP接口                              |
| wget [http://example.com/file.zip](http://example.com/file.zip) | 下载文件                                  |
...
| 命令                           | 说明               |
| ---------------------------- | ---------------- |
| echo $JAVA\_HOME             | 查看JAVA\_HOME环境变量 |
| echo $PATH                   | 查看PATH环境变量       |
| export JAVA\_HOME=/opt/jdk11 | 临时设置环境变量         |
