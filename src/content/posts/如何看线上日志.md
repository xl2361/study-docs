...
如果项目能访问外网，可以用AI 工具读你的项目代码，找日志配置文件和路径。

**方法四：问同事 / 看文档**

有些团队会有部署/运维文档，里面有各个服务的日志路径。问同事：这个服务的日志在服务器哪个目录？
...
主要看当天的**主日志文件** (如 `zcloud-learning.log`)。如果配置了单独输出 error 日志，报错时先看 error 日志，因为它只记录错误，不用在一堆 info 日志里翻。
...
还要注意：**日志不会永远保留**。运维一般会配置日志保留策略，只保留 7/15 天。拖久了日志可能就没了。
...
如果日志刷得很快（比如有定时任务在跑），日志一下子就刷过去了，这时候加过滤：
...
- 问题是业务逻辑异常 → 过滤业务关键词，比如订单号：
  ```bash
  tail -f zcloud-learning.log | grep "1008"
  ```
...
- **想用编辑器的搜索功能** → 下载到本地：
  ...
  用 VS Code 或 Notepad++ 打开，搜索。前提是公司允许把日志下载到本地——安全要求高的团队会限制，不确定的话先问一下。
