...
- 实现简单，浏览器自动管理 Cookie
- 状态可控，服务端可以主动让 Session 失效（退出登录、修改密码），可以随时查看和管理所有在线用户
...
验证用户名密码
        ↓
验证通过，生成JWT （后端开始组装JWT：Header、Payload、Signature）
        ↓
返回JWT给客户端
        ↓
后续请求携带JWT
...
- **无法控制状态**：JWT 过期前一直有效，不能主动失效。不好处理修改密码、踢下线等场景
...
| 方案   | 说明                                                                |
| ---- | ----------------------------------------------------------------- |
| 主动失效 | 用户退出登录或改密码时，把对应 JWT 放入Redis 黑名单，校验时先查黑名单                          |
| 续期   | 登录时同时返回一个短期 Token 和一个长期 Refresh Token，Token 过期后用 Refresh Token 换新 |


#### 2.8 Token 和 Session 的核心区别(了解)
...
- 新增数据时，要记录这条数据是谁创建的（create\_by 字段）
- 修改数据时，要记录是谁改的（update\_by 字段）
- 操作日志时，记录"张三在 2024-01-01 删除了某条数据"
- 查询时只返回当前用户自己的数据，比如"我的订单"

这些场景的共同点：**后端需要知道当前请求是谁发的**，而不是让前端来告诉后端。

你要记住获取登录用户：

1. **通过 Session / Token 实现**，项目中一般有封装好的工具类，搜不到就问同事：项目里有没有获取当前登录用户的方法？没有再自己写。
2. **不要让前端传用户 ID**（除非你组长要你这样做）。
...
核心思路：拦截请求 → 解析凭证中的用户信息 → 存入当前线程 → 后续随时调用

**1）、拦截请求**

请求到达后端，会先经过全局拦截器(Filter / Interceptor)。它会检查请求中有没有登录凭证(Token、SessionID)。

**2）、解析用户信息**

- 如果是Session： 后端会根据SessionID，从Tomcat 或 Redis中找对应的用户对象。
- 如果是Token： 后端会解析JWT，取出userId等用户信息，要完整用户数据可以去 Redis 查。

**3）、存入当前线程（ThreadLocal）**

把用户信息存入ThreadLocal。后续在Controller、Service中调用就行：

```java
SecurityUtils.getUser()   //Spring Security 的 SecurityContext
```
...
| 用工具类解析 Request          | 常见   | SecurityUserUtil.getCurrentUserId(request)             | 直接从请求头解析Token |
...
上面介绍的若依是典型的 RBAC 权限模型，适合角色多、权限细的场景。但有些项目并不需要精细的权限控制，只需要区分"管理员"和"普通用户"，这种情况下在代码里加一层判断就行。

#### 2.1 用户表加一个角色字段

直接在用户表里加一个角色字段，来标识用户类型：
...
登录后，这个 `role` 字段会随着用户信息一起存到 Token 或 Session 中。后续请求时，直接判断角色即可。
...
更好的做法，是通过拦截器（Filter / Interceptor）统一处理：
...
所有以 `/admin/` 开头的接口，普通用户访问角色不匹配，会被拦截直接返回 403。这样不用每个接口都写判断。
...
它是一个**授权框架**，用于**用户在不给第三方应用密码的情况下，授权它访问自己的资源。**

最常见的场景就是"第三方登录"，如：

- 用微信扫码登录某个网站
- 用 GitHub、Google 账号登录某个网站
...
> 简单来说：**OAuth 2.0 是"流程"，JWT 是"格式"。** OAuth 2.0 流程中产生的 Token 可以选用 JWT 格式。
...
想要知道你们项目怎么放行接口的，就全局搜索登录接口，去看它怎么放行的，或者让 AI 告诉你。
...
> **好处**：放行配置写在接口上，不用跑到配置类里去找。若依框架就是用的这种注解方式来放行。

先定义一个 `@Anonymous` 注解，加在接口上，表示不需要登录：
...
>
> 按 F12 进浏览器开发者：如果不确定某个接口的认证参数怎么传，先登录公司的平台，切换到网络面板，然后在页面上随便操作，观察其他接口是怎么传 Token/Cookie 的——请求头叫什么名字、格式是什么（比如 `Bearer xxx` 还是直接传值）。照着别的接口的传参方式来就行。这是最快的方式，比自己猜或者翻代码都快。
