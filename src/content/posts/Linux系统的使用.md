...
vi和vim用来在Linux中创建、查看或者编辑文本文件，vim增加了语法高亮、多级撤销、可视化、插件等。

**三种模式：**

```
启动vim → 默认进入命令/普通模式
命令模式 → 编辑模式：按 i/a/o ， 按Esc回到命令模式
命令模式 → 底行/命令行模式：按 : ， 按Esc回到命令模式
```


| 模式        | 说明        | 常用操作                                                                      |
| --------- | --------- | ------------------------------------------------------------------------- |
| **普通模式**  | 打开文件的默认模式 | `gg`：跳到文件首行<br>`G`：跳到文件末行<br>`yy`：复制当前行<br>`p`：粘贴<br>`dd`：删除当前行<br>`u`：撤销 |
| **编辑模式**  | 编辑文件内容    | `i` ：光标前插入<br>`a` ：光标后插入<br>`o` ：下方新开一行                                   |
| **命令行模式** | 保存、退出、搜索等 | `:` 进入，(即 shift+: )                                                       |
...
| --------------------- | --------------- |
| `mkdir data`          | 创建data目录        |
| `mkdir -p data/mysql` | 递归创建目录          |
| `mkdir /opt/data`     | 在opt目录下创建data目录 |
| `cp 1.jar 11.jar`     | 复制文件并重命名（备份常用）  |
| `cp -r dir1 dir2`     | 递归复制目录          |
| `cp 1.jar /opt/`      | 复制文件到指定目录       |
...
| `head / tail -n 50 file.log`    | 查看文件前/后 50行             |
| `tail -f file.log`              | **实时查看**文件末尾内容（查看日志常用）  |
...
Linux中文件/目录的权限：所有者、同组用户、其他用户 权限。
...
| `lsof -i :3306`                        | 查看占用3306端口的进程（精准查）           |
| `ss(`~~netstat~~`) -tunlp ｜ grep 8080` | 查看占用8080端口的进程（全列出再过滤）        |
...
# 4. 后台启动Java应用  nohup：不挂断、> app.log：输出到日志、2>&1：错误也写入刚才的日志、&：挂后台
...
| 命令                            | 说明               |
| ----------------------------- | ---------------- |
| `echo $JAVA_HOME`             | 查看JAVA\_HOME环境变量 |
